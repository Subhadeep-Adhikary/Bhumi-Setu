  ...
  return (
    <Box sx={{ width: '100%', minHeight: '100vh', bgcolor: '#ecebe6', p: 0 }}>
      <Box sx={{ width: '100%', p: 3, boxSizing: 'border-box', display: 'flex', flexDirection: 'column' }}>
        ...
        {/* ====== YOUR EXISTING DASHBOARD PART - KEPT SAME ====== */}
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(3, minmax(0, 1fr))' }, gap: 4, mb: 5, width: '100%' }} name="boxes of statistics">
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: 'repeat(2, minmax(0, 1fr))' }, gap: 3, alignItems: 'stretch', mb: 3 }}>
          <Box sx={{ bgcolor: '#eef6e6', borderRadius: '26px', p: 3, border: '1px solid #dfead8', boxShadow: '0 10px 30px rgba(0,0,0,0.06)' }}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'flex-end', mt: 2 }}>
              <Link
                ...
                className="inline-flex items-center justify-center rounded-[16px] bg-[#16633d] px-8 py-3 text-lg font-extrabold text-white no-underline shadow-[0_6px_16px_rgba(22,99,61,0.3)] transition-colors hover:bg-[#134f31] focus:outline-none focus:ring-2 focus:ring-[#1a8a64] focus:ring-offset-2"
              >
                View All
              </Link>
            </Box>
          ...
          <Box sx={{ minHeight: 220, bgcolor: '#ffffff', borderRadius: '26px', p: 3, border: '1px solid #e9ece7', boxShadow: '0 10px 30px rgba(0,0,0,0.06)' }}>
            ...
            {acquisitionStatuses.map((item) => (
              <Box key={item.label} sx={{ mb: 2.8 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.8 }}>
                  <Typography sx={{ fontSize: 15, fontWeight: 600, color: '#243a2e' }}>{item.label}</Typography>
                ...
                <Box sx={{ height: 14, bgcolor: '#edf2ea', borderRadius: 99, overflow: 'hidden' }}>
                  <Box sx={{ width: `${item.value}%`, height: '100%', bgcolor: item.color, borderRadius: 99 }} />
                </Box>
              </Box>
            ))}
          </Box>
        ...
        <Box sx={{ width: '100%', bgcolor: '#eef6e6', borderRadius: '26px', p: 4, border: '1px solid #dfead8', boxShadow: '0 10px 30px rgba(0,0,0,0.06)', transition: 'transform 0.2s ease, box-shadow 0.2s ease', '&:hover': { boxShadow: '0 14px 34px rgba(22,99,61,0.12)' } }}>
          ...
          <Box sx={{ display: 'grid', gridTemplateColumns: { xs: 'repeat(3, 1fr)', md: 'repeat(6, 1fr)' }, gap: 3, width: '100%' }}>
            {stateSnapshot.map((state) => (
              <Box key={state.name} sx={{ textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', p: 1.5, borderRadius: 3, bgcolor: 'rgba(255,255,255,0.25)' }}>
                <Donut3D value={state.value} c1={state.c1} c2={state.c2} />
                <Typography sx={{ mt: 2, fontWeight: 800, fontSize: 15, color: '#11261c' }}>{state.name}</Typography>
                <Typography sx={{ fontSize: 13, color: '#6a8a7c', fontWeight: 600, mt: 0.2 }}>{state.projects}</Typography>
              </Box>
            ))}
